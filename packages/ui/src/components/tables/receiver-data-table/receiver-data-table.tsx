...
export const ReceiverTable = () => {
  ...
  return (
    <div className="max-h-[2160px] w-full no-scrollbar relative overflow-auto">
      <table className="w-full text-sm text-center text-gray-500">
        ...
        <hr />
        <tbody>{DATA.map((item, index) => TRD({ item, index }))}</tbody>
      </table>
    </div>
  );
};
...
const TRD = ({ item, index }: { item: data; index: number }) => {
  ...
  return (
    <tr
      ...
      key={index}
      className="bg-inherit dark:bg-inherit hover:bg-slate-400 hover:dark:bg-slate-950 border"
    >
      ...
      <td className="px-6 py-4">
        {TRANSMISSION_STREANGTH(item.transmission_streangth)}
      </td>
